...
def ENSO_acrossRuns(adf, clobber=False, search=None):
    ...
    obs_ds = xr.open_dataset('/projects/NS2345K-datalake/adagj/ADFout/ensometrics/ENSOmetrics_Obs.nc').load()
    ...
    # - - - - - - - - - - - - - - - - - - - - - - - 
        ...
        print(f'ADA: case: {sst_case_weighted}\n')
        print(f'ADA: obs: {obs_ds.sst_obs_weighted}\n')
        print(f'ADA: ocnmask: {np.size(ocnMask[0,:,:])}\n')

        obs_sst_weighted = obs_ds.sst_obs_weighted.reindex_like(sst_case_weighted, method="nearest", tolerance=1e-6)

        print(f'ADA: obs: {sst_case_weighted - obs_sst_weighted}')

        sst_bias[:,:,:] = (sst_case_weighted - obs_sst_weighted) * ocnMask[0,:,:]
        ...
        file_path = "/projects/NS2345K-datalake/adagj/ADFout/ensometrics/ENSOmetrics_NorESM3dev.nc"

        if os.path.exists(file_path):
            with xr.open_dataset(file_path) as dev_ds:
                newDS = xr.concat([dev_ds, thisDev_DS], dim="case")
            ## Save that new DS out to dev_ds and reload 
            dev_ds.close()
        else:
            newDS = thisDev_DS  # Start fresh if file doesn't exist


        # Save or overwrite the file
        #newDS.to_netcdf(file_path, mode='w')
        #dev_ds   = xr.open_dataset("/projects/NS2345K-datalake/adagj/ADFout/ensometrics/ENSOmetrics_NorESM3dev.nc")
        #newDS    = xr.concat([dev_ds, thisDev_DS], dim="case")

        newDS.to_netcdf("/projects/NS2345K-datalake/adagj/ADFout/ensometrics/ENSOmetrics_NorESM3dev.nc", mode='w')
