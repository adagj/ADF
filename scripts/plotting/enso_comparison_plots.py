...
def enso_comparison_plots(adfobj):
    ...
    #Grab saved files
    obs_ds   = xr.open_dataset('/projects/NS2345K-datalake/adagj/ADFout/ensometrics/ENSOmetrics_Obs.nc')
    #noresm2lm_ds = xr.open_dataset("/projects/NS2345K-datalake/adagj/ADFout/ensometrics/ENSOmetrics_NorESM2-LM.nc")
    noresm2mm_ds = xr.open_dataset("/projects/NS2345K-datalake/adagj/ADFout/ensometrics/ENSOmetrics_NorESM2MM.nc")
    dev_ds   = xr.open_dataset("/projects/NS2345K-datalake/adagj/ADFout/ensometrics/ENSOmetrics_NorESM3dev.nc")
...
#                          J   F   M    A  M   J   J   A    S   O    N  D
    ...
    for iNino in range(len(ninoSelString)):
        ...
        #noresm2lm_xCenter = len(dev_ds.case.values)+1.5
        #noresm2mm_xCenter = len(dev_ds.case.values)+3
        obs_xCenter   = len(dev_ds.case.values)+4

        #offset = np.linspace(noresm2lm_xCenter-0.5, noresm2lm_xCenter+0.5, len(noresm2lm_ds.event.values))
        # for iEvent in range(len(noresm2lm_ds.event.values)):
        #     case_max = np.nanmax(noresm2lm_ds[pltVar].values[iEvent,:])
        #     case_min = np.nanmin(noresm2lm_ds[pltVar].values[iEvent,:])
        #     # Weighted mean
        #     weights = ( daysPerMonth / daysPerMonth.sum() )
        #     weighted_mean = (noresm2lm_ds[pltVar].values[iEvent,:] * weights).sum() / weights.sum()
        ...
        # offset = np.linspace(noresm2mm_xCenter-0.5, noresm2mm_xCenter+0.5, len(noresm2mm_ds.event.values))
        # for iEvent in range(len(noresm2mm_ds.event.values)):
        #     case_max = np.nanmax(noresm2mm_ds[pltVar].values[iEvent,:])
        #     case_min = np.nanmin(noresm2mm_ds[pltVar].values[iEvent,:])
        #     # Weighted mean
        #     weights = ( daysPerMonth / daysPerMonth.sum() )
        #     weighted_mean = (noresm2mm_ds[pltVar].values[iEvent,:] * weights).sum() / weights.sum()
        ...
        #ticks = np.append(1+np.arange(len(dev_ds.case.values)), noresm2lm_xCenter)
       # ticks = np.append(ticks, noresm2mm_xCenter)
        #ticks = np.append(ticks, obs_xCenter)
        ticks = np.append(1+np.arange(len(dev_ds.case.values)), obs_xCenter)
        ticks = np.append(ticks, obs_xCenter)

        #tickLabels = np.append(dev_ds.case.values, 'NorESM2-LM')
        #tickLabels = np.append(tickLabels, 'NorESM2-MM')
        #tickLabels = np.append(tickLabels, 'HadiSST')
        tickLabels = np.append(dev_ds.case.values, 'NorESM3dev')
        tickLabels = np.append(tickLabels, 'HadiSST')
    ...
    #axs.plot(np.full([len(noresm2lm_ds.event.values)], 'NorESM2-LM'), noresm2lm_ds.nino34_transMonth, 'o', color='mediumpurple', alpha=0.4)
    #axs.plot(np.full([len(noresm2mm_ds.event.values)], 'NorESM2-MM'), noresm2mm_ds.nino34_transMonth, 'o', color='orange', alpha=0.4)
    ...
    ticks = np.arange(len(dev_ds.case.values)+1)

    #tickLabels = np.append(dev_ds.case.values, 'NorESM2-LM')
    #tickLabels = np.append(tickLabels, 'NorESM2-LM')
    #tickLabels = np.append(tickLabels, 'NorESM2-MM')
    #tickLabels = np.append(tickLabels, 'HadiSST')
    tickLabels = np.append(dev_ds.case.values, 'HadiSST')
    #tickLabels = np.append(tickLabels, 'HadiSST')
    print(f'ADA: ticks: {np.size(ticks)}')
    print(f'ADA: ticklabels: {np.size(tickLabels)}')
    axs.set_xticks(ticks)
   ...
   # + + + + + + + + + + + + + + + + + + + +
    ...
    axs[0].invert_yaxis()
    #axs[0].plot(noresm2lm_ds.nino34_zeroContour, np.full([len(noresm2lm_ds.event.values)], 'NorESM2-LM'), 'o', color='mediumpurple', alpha=0.4)
    #axs[0].plot(noresm2mm_ds.nino34_zeroContour, np.full([len(noresm2mm_ds.event.values)], 'NorESM2-MM'), 'o', color='orange', alpha=0.4)
    ...
    axs[1].invert_yaxis()
    #axs[1].plot(noresm2lm_ds.nino34_0p5Contour, np.full([len(noresm2lm_ds.event.values)], 'NorESM2-LM'), 's', color='mediumpurple', alpha=0.4)
    #axs[1].plot(noresm2mm_ds.nino34_0p5Contour, np.full([len(noresm2mm_ds.event.values)], 'NorESM2-MM'), 's', color='orange', alpha=0.4)
    ...
    # sst_noresm2mm_raw_season = sst_noresm2mm.sel(season=selSeason)
    ...
    ## Add CESM ensembles
    axs.scatter(noresm2mm_ds.sst_raw.sel(season=selSeason,lat=slice(-5,5), lon=slice(120,280)).mean(dim='lon').mean(dim='lat').values, 
                noresm2mm_ds.nino34_transMonth.values, marker='s', c='orange', alpha=0.8)
    axs.text(np.nanmean(noresm2mm_ds.sst_raw.sel(season=selSeason,lat=slice(-5,5), lon=slice(120,280)).mean(dim='lon').mean(dim='lat').values)*1.001, 
            np.nanmean(noresm2mm_ds.nino34_transMonth.values), 'NorESM2-MM', color='orange', alpha=0.8, fontsize=12)


    #axs.scatter(noresm2lm_ds.sst_raw.sel(season=selSeason,lat=slice(-5,5), lon=slice(120,280)).mean(dim='lon').mean(dim='lat').values, 
    #            noresm2lm_ds.nino34_transMonth.values, marker='^', c='mediumorchid', alpha=0.8)
    #axs.text(np.nanmean(noresm2lm_ds.sst_raw.sel(season=selSeason,lat=slice(-5,5), lon=slice(120,280)).mean(dim='lon').mean(dim='lat').values)*0.998, 
    #         np.nanmean(noresm2lm_ds.nino34_transMonth.values), 'NorESM2-LM', color='mediumorchid', alpha=0.8, fontsize=12)
